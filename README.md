...
Add `smooth-shadow-ambient` when a floating surface needs the same visible shadow on every side. The utility recenters each layer, converts its original downward offset into spread, and slightly increases opacity to preserve the bottom profile without overlapping directional copies. It composes with both ringless and ringed shadows. Without this class, the original directional shadow is unchanged.
